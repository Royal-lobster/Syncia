...
export const useChatCompletion = ({ model, apiKey, systemPrompt }: OpenAIStreamingProps) => {
	const systemMessage = createChatMessage({
		content: systemPrompt || "",
		role: ChatRole.SYSTEM,
	});

	const [storedMessages, setStoredMessages] = useStorage<ChatMessage[]>("CHAT_MESSAGES", [systemMessage], "local");
	...
	const submitQuery = React.useCallback(
		(newMessages?: ChatMessageParams[]) => {
			...
			const payload = JSON.stringify({
				model,
				messages: updatedMessages.filter((m, i) => updatedMessages.length - 1 !== i).map(officialOpenAIParams),
				stream: true,
			});
