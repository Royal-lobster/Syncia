...
function createIdealGraph(assetGraph, config, entries) {
  ...
  let assetSet = _utils().BitSet.from(assets);
  ...
  for (let nodeId of bundleRootGraph.topoSort(_graph().ALL_EDGE_TYPES)) {
    ...
    if (bundleRoot.bundleBehavior === 'isolated') {
      ...
      available = (0, _nullthrows().default)(ancestorAssets.get(bundleRoot)).clone();
    ...
    for (let childId of children) {
      ...
      let currentChildAvailable = isParallel ? _utils().BitSet.union(parallelAvailability, available) : available;
      ...
      if (isParallel) {
        for (let reachableNodeId of reachableRoots.getNodeIdsConnectedFrom(reachableRoots.getNodeIdByContentKey(child.id))) {
          let asset = (0, _nullthrows().default)(reachableRoots.getNode(reachableNodeId));
  ...
  for (let asset of assets) {
    ...
    if (reachable.length > config.minBundles) {
      ...
      if (bundleId == null) {
        ...
        let sharedInternalizedAssets = firstSourceBundle.internalizedAssets ? firstSourceBundle.internalizedAssets.clone() : assetSet.cloneEmpty();
